...
const MovieUserListCategoryRow = ({ title, movies, onMovieSelect, onRemove }) => {
                ...
                {movies && movies.length > 0 ? (
                    movies.map((movie) => (
                        <div key={movie.id} className="min-w-[140px] max-w-[240px] flex-shrink-0" onClick={() => onMovieSelect(movie)}>
                            <UserMovieCard movie={movie} listName={title} onRemove={onRemove} />
                        </div>
                    ))
                ) : (
...
export default MovieUserListCategoryRow;
